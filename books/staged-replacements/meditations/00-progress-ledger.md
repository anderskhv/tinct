...
- 2026-09-12 — **Book XII round 1 reviewed, and Book XII ACCEPTED (steps 6–8).**
  Verdict *accept after corrections*: 0 substantive, 11 minor, 6 optional, 23
  paragraphs with no material issue, each of the 36 covered exactly once. **All
  eleven minor findings applied and all six optional findings answered and
  applied; none declined** — ten substitutions in eight paragraphs (XII.3,
  XII.4, XII.5, XII.15, XII.18, XII.26, XII.34, XII.36) and seven record
  corrections. `book12/candidate-v2.json` sha256 `8510a04f…`, built by
  `scripts/build_book12_v2.py`; `book12/changes-v1-to-v2.md` and
  `book12/ACCEPTANCE.md` written; `README.md`, `continuity.md`,
  `provenance.json`, `manifest.json`, `review-instructions.md` and
  `PROVENANCE.md` §4 updated. **Every substantive call was confirmed** — both
  departures from PG's letters, XII.3's D11 route, XII.17's whole-clause
  textual-doubt application, XII.27's "[or Rufus at Velia]", "pancratiast",
  XII.23's repairs, D13 firing nowhere, the three PG-over-SE calls and the XII.12
  stray "18". The one ruling against the draft, **4.1**, rules a rule: XII.4 now
  reads "will" in both halves of its comparison, so **the candidate keeps none of
  Long's ten "shall / shalt"**. **D13's textual-doubt sentence widened to "a word
  or a clause"** (finding C3), naming XI.26 and XII.17. Finding **2.1** also
  repaired the mechanical check: the comma tally is now asserted by **position**,
  not by net count per paragraph, which is what let XII.2's one-for-one swap pass
  unseen. Finding **C1** put the nine-space footnote leak at **XII.4, not XII.3**
