...
| `thence` | `from there` (B6-P001) | **`context-rendered`** | `moved them thence and settled them in Scheria` -> `from there` (B06-P001) is spatial; `thence go on to Sparta` -> `then on to Sparta` (B01-P019) is a step in an itinerary, where English says `then`. `moved them then` is wrong and `from there go on to Sparta` reads as though Pylos were the obstacle. |
