...
(worker 7, **drafting role**). **Books 1–6 are accepted. Book 7 is drafted and
frozen and is waiting on an independent review.** The package's checks now
**run**, and that is the largest thing that changed.
...
| **7** | **3 — frozen draft, review NOT run** | — | all 29 | **0.93943** | +25.2% | **+7.5%** | **0.01277** | 30 → 14 |
...
## Book 7 — drafted and frozen, waiting on the review
...
1. **Book 7's independent review.** A separate session, its own worktree,
   writing only in `book07/review/`. **Verify the source by an ELEVENTH kind of
   rule** and **audit it before trusting it**: six audits have now been run in
   this package and **all six failed the rule as first written**; Book 6's
   review rule failed three times and Book 7's failed three times.
2. **Then Book 8**, in numerical order, by the full eight steps.
3. **Report five numbers with the basis on every one**: retention, raw D17 rate,
...
5. **Run `scripts/checks.py --all`, `scripts/compound_drift.py` and
...
6. **Anders still owes two decisions** — `00-progress-ledger.md` **A4**(ii) (a
...
**An eleventh has to find another channel again.** Nothing so far has used the
