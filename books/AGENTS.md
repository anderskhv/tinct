...
- Quoted speech, verse, and formulas: modernize them the same as surrounding prose. Quotation marks are not an exemption — an oath, contract formula, definition, epigraph, or line of dialogue left in archaic spelling/syntax while the prose around it is modern is a defect, not a stylistic choice. Preserve meaning, imagery, rhetorical function, and recognizable recurring wording. When the source's own discussion is about the exact wording (a character remarking that one phrasing sounds different from another), preserve the distinction that discussion depends on, and add a brief clarification only if needed. This applies only to quotations embedded in the source you were given — never substitute wording from another translation or a modern copyrighted edition, even for a quotation you recognize (a well-known oath, a Bible verse).
