...
4. Footnote-slot paragraphs are never deleted (they exist in original-en and modern-da and alignment depends on them). In modern-en the slot carries the original French, prefixed `*`, so a reader who wants the French has it and nothing is duplicated: main paragraph = English + (in French), slot = `* Contez nous çela, Vicomte.` Slots whose source footnote is not a translation but a gloss ("Old style date.", "An esaul is a captain of Cossacks.") keep the gloss text, also prefixed `* ` (amended 2026-09-18 after the ch 1–153 scan: chapter-only readers met bare "(Denisov.)" lines as orphaned notes; with every slot starting `* ` the app can style all slots as footnotes with one rule). `assemble.py` normalises any parenthesised slot on assembly.
...
|---|---|---|---|
| Duplicate title "Book Two — Chapter 1" (ch 28 and 29), duplicate "Chapter 5" (32, 33), no Chapter 2 | structural | `edition_checks.py` title-diff/title-duplicate flags; introduced by the early-flags batch | **DONE in baseline v2**: modern-en titles mirror the source in every chapter. The source mislabel of ch 28 stays for the structural repair below |
| 16 book-final chapters mislabelled in source | structural (source) | title-sequence flags (32) and title-duplicate (7) on the assembled file, all source-side | Separate repair, all three editions. Open. |
| Single-quote dialogue in ch 283, 285, 286, 289, 292, 293, 295, 296, 302, 303, 305, 306, 307, 308, 309 | consistency | quote-style flags | **DONE in baseline v2**, verified no quote-within-quote damage |
| Bolkonski (27), Nesvitski (21), Hélène (37, ch 50–51), Kozlovski, Kamenski, Kirill (4), Mikhailovna (1), Andrei (1, ch 212) | consistency | name-variant flags | **DONE in baseline v2** by character identity (Bolkonski kept per cards and Maude; Nesvitsky, Kozlovsky, Kamensky, Helene, Cyril, Mikhaylovna, Prince Andrew) |
| Three French conventions; 33 bare footnote-slot lines; 10 tags mid-sentence; 4 duplicated translations | consistency | footnote-slot-bare and bracket-tag flags | **DONE**: French pass, 6 batches + sweep + 7 drafted-overlap chapters, all Opus-reviewed; 0 French flags on the assembled file. Gap found: "French inline + orphan `*` + English in slot" is invisible to the flag set; add a flag before the ch 1–153 scan |
| Em-dash spacing alternates by chapter; diacritics stripped from Olmütz, Schön Grabern | consistency | close-read | Open. Scripted normalisation after the diacritics decision; unspaced em dashes applied in every drafted chapter |
| Minor fidelity drift, 1–4 items in about half of chapters (dropped clause, softened word, added flourish); one grammar error ch 203 p38 | fidelity | 24-chapter independent close-read 2026-09-18 | Caught by Gate B in the 43 chapters that went through the procedure; the unscanned zone ch 1–153 still carries it |
| Essays ch 338–365 faithful but structurally unchanged (mean sentence 23.5 words, 164 sentences over 50 words); ch 154–278 largely near-verbatim Maude | accessibility | sentence and similarity metrics | **DONE for ch 154–365**: Gate A scan, 43 chapters repaired and accepted (`repair/ACCEPTED.md`). 25 chapters in 158–278 remain LIGHT on the similarity classifier while passing Gate A: policy question in `MODERN-EN-REPAIR-STATUS.md` |
