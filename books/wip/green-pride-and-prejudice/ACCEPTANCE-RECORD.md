...
| Whole-book checks | All 2,060 paragraphs | Lead, programmatic (`CONSISTENCY-CHECK.md`) | Structure, the mandatory `classify-modern-en.py --gate` (PASS, similarity 0.672), quote, italic and dash balance against the source, proper names and numbers against the source, key-term consistency, anachronism sweep, `books/audit-truncation.py` and `books/content-verify.py` (run with local paths) | Clean, or explained case by case |
