...
2. **Audio.** English modern-en audio exists in R2. For example,
   `/api/audio-manifest?path=frankenstein/modern-en/ch1/manifest.json` returns
   200. The 301 changed paragraphs above need regenerated audio and manifests.
   Word-timing sidecars for those chapters also need regenerating, if any exist.
