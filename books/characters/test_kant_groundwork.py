...
from build_kant_groundwork import compile_package,BASE
from build_pilot import ROOT,normalized,point
from lookup_reference import resolve,gallery
class KantContractTests(unittest.TestCase):
 @classmethod
 ...
 def test_footnotes_and_gospel_identity(self):
  for d in self.asset['editions'].values():
   for id,pi in [('sulzer',11),('hutcheson',103),('jesus',4)]:
    self.assertTrue(any(m['characterId']==id and m['chapterNumber']==3 and m['paragraphIndex']==pi for m in d['mentions']))
 def test_reference_scope(self):
  for ed,d in self.asset['editions'].items():
   self.assertEqual(self.report['editions'][ed]['omittedEntities'],[]);self.assertEqual(len(d['characters']),7)
