...
Revision 2 (2026-09-11) of the pinned ``f5b23de7`` helper. Everything the
acceptance results were measured against is unchanged except the comparison
between expected edition tokens and recognised tokens, which now:
  ...
  * strips never-spoken markup from the EXPECTED side only — underscore
    ...
    (``--``, which separate words rather than being words);
  * joins recogniser-split hyphen compounds (expected ``wage-labour``, heard
    ``wage`` ``labour``) into one heard token whose timing runs from the first
    piece's start to the last piece's end.

The 0.85 gate, the timestamps and the interpolation rule are untouched; no
timestamp or confidence is synthesised. The provenance record is
``tools/audio-highlight/aligner/PINS.md``. A paragraph with none of that markup
aligns byte-for-byte as under revision 1 (``test_normalisation.py`` proves it).
"""
...
def _join_split_compounds(
    ...
    """Within each unresolved ``replace`` block, join adjacent heard pieces that
    spell one of the block's hyphen compounds.  Nothing outside such a block
    is touched, so an alignment that already resolved stays as it was."""
    compounds = {
        i: pieces
        for i in spoken
    ...
    for tag, i1, i2, j1, j2 in opcodes:
        if tag != "replace":
            continue
        block = [spoken[i] for i in range(i1, i2) if spoken[i] in compounds]
        if not block:
            continue
        j = j1
        while j < j2:
...
def align_tokens_detailed(
    ...
    """Revision-2 alignment with full provenance (see module docstring)."""
    ...
    for ei, hj in observed.items():
        aligned[ei] = {
            "text": expected_tokens[ei],
            "start": round(comparison[hj].start, 3),
            "end": round(comparison[hj].end, 3),
        }
    ...
    return DetailedAlignment(words, stats, opcodes, spoken, unspoken, observed, comparison, merges)
...
def align_tokens_with_stats(
    ...
    not make a low-quality transcript look complete.  Revision 2: see
