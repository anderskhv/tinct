...
import re
from spoken_policy import is_scene_separator,validate_map
...
def main():
 ...
 for key in a.key:
  ...
  entries=[p for p in manifest['paragraphs'] if p['paragraph']>=0]
  nonspoken=[dict(index=i,text=text) for i,text in enumerate(chapter['paragraphs']) if is_scene_separator(text) and i not in {p['paragraph'] for p in entries}]
  assert validate_map(dict(paragraphs=[dict(index=p['paragraph']) for p in entries],text_paragraph_count=len(chapter['paragraphs']),nonspoken=nonspoken)),key+' paragraph-map mismatch'
  def fetch(p):
   assert re.fullmatch(r'p[0-9]+\.mp3',p['file'])
  ...
  result.append(dict(key=key,group='complete-edition-repair',nonspoken=nonspoken,title=chapter['title'],text_sha256=sha(raw),manifest_sha256=sha(mr),text_paragraph_count=len(chapter['paragraphs']),paragraphs=sorted(paragraphs,key=lambda p:p['index'])))
