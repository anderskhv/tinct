...
Revision 7 (2026-09-18) of the pinned ``f5b23de7`` helper, built on revision 6.
Scoring is identical to revision 6. The only change is that ``clean_text`` can
skip whole-paragraph heading detection so ``restore_source_tokens`` can map
prefixes of prose monotonically. Prefixes such as ``45. I`` (from Hume
``45. I shall add…``) match the numbered ALL-CAPS heading shape and, under
revision 6, emptied the acoustic prefix mid-restore. Heading-only paragraphs
still collapse when ``strip_headings`` is left at its default.

Everything earlier pins measure is unchanged except the comparison between
expected edition tokens and recognised tokens, which now also:

  * unwraps stage-direction wrappers the recogniser does not say
    (``[_Exeunt …_]`` → ``Exeunt …``) while keeping the spoken inner words;
  * unwraps WEB editorial brackets (``[of]``, ``[and]``, ``[their]``) to the
    inner words, and drops superscript verse markers from the acoustic
    denominator the way revision 2 already drops ``[28]``;
  * drops ALL-CAPS speaker labels (``AMIENS.``, ``LE BEAU.``) and ALL-CAPS
    names inside Enter / Exeunt / Exit / Alarum lines from the acoustic
    denominator — ASR hears the dialogue, not the printed cue spelling;
  * treats a few common archaic elisions as comparison equivalents
    (``prepar'd`` ↔ ``prepared``, ``Hear'st`` ↔ ``hearest``, ``Th'art`` ↔
    ``thou art``) without rewriting emitted source words;
  * drops whole-paragraph unspoken chapter/section headings the way cues are
    dropped — numbered ALL-CAPS labels (``1. REACTIONARY SOCIALISM``),
    lettered italic subsections (``_C. German, or “True,” Socialism_``),
    short Roman-numeral titles, ``Chapter``/``Part``/``Section`` labels, and
    short ALL-CAPS or italic title lines. Ordinary prose, including numbered
    manifesto sentences such as ``I. Communism is already acknowledged…``,
    stays in the denominator. Headings that share a paragraph with prose are
    not stripped.
...
confidence is synthesised. Source-word emission stays the revision-4 rule:
acoustic normalisation is separate from emitted source tokens; unspoken
footnote labels, superscript verse markers and heading-only paragraphs remain
zero-span source words.
The provenance record is ``tools/audio-highlight/aligner/PINS.md``.
"""
...
# Revision 5: structural cues the recogniser does not spell the way the
# edition prints them. Applied on the acoustic side only; emitted source
# words still come from the raw edition split (see trial.restore_source_tokens).
WEB_EDITORIAL_BRACKET = re.compile(r"\[([a-z][a-z ]*)\]")
STAGE_WRAP = re.compile(r"\[_([^[\]]*?)_\]")
SPEAKER_LABEL = re.compile(
    r"^((?:[A-Z]{2,}(?:'[A-Z]{1,2})?\s+)*[A-Z]{2,}\.)(?:\s+|$)",
)
STAGE_DIRECTION_START = re.compile(
    r"^(?:\[_)?(?:Enter|Re-enter|Exeunt|Exit|Alarum|Flourish|Excursions)\b",
    re.IGNORECASE,
)
STAGE_KEEP = frozenset({
    "ENTER", "RE", "EXEUNT", "EXIT", "ALARUM", "FLOURISH", "EXCURSIONS",
    "AND", "OR", "THE", "WITH", "TO", "ALL", "BUT", "FROM", "INTO",
    "FOR", "NOT", "NOR", "AS",
})
ARCHAIC_PAST = re.compile(r"'d\b", re.IGNORECASE)
ARCHAIC_SECOND = re.compile(r"'st\b", re.IGNORECASE)
SUPERSCRIPT_TOKEN = re.compile(r"^[⁰¹²³⁴⁵⁶⁷⁸⁹]+$")

# Revision 6: whole-paragraph headings the narrator does not say. Conservative
# on purpose — only short label-like lines, never ordinary prose.
HEADING_MAX_WORDS = 12
HEADING_PARTICLES = frozenset({
    "a", "an", "and", "as", "at", "by", "for", "from", "in", "into",
    "nor", "of", "on", "or", "the", "to", "vs", "versus", "with",
})
HEADING_SECTION_LABEL = re.compile(
    r"^(?:Chapter|Part|Section|Book|Canto|Volume)\b",
    re.IGNORECASE,
)
HEADING_NUMBERED_ALLCAPS = re.compile(
    r"^\d+\.\s+[A-Z0-9 ,;:'\"“”‘’\-—/]+$",
)
HEADING_LETTERED = re.compile(r"^_?[A-Z]\.\s+\S")
HEADING_ROMAN = re.compile(r"^[IVXLCM]+\.\s+\S")
HEADING_ALLCAPS_LINE = re.compile(r"^[A-Z0-9 ,;:'\"“”‘’\-—/.]+$")
HEADING_PROSE_END = re.compile(r"[.?!][\"”']?$")


def _unwrap_heading_marks(text: str) -> str:
    text = text.strip()
...
def _looks_like_title(text: str) -> bool:
    """True when every content word is title-case or ALL-CAPS."""
    ...
    titled = 0
    for word in words:
        if word.lower() in HEADING_PARTICLES:
            continue
        content += 1
        if word[:1].isupper():
            titled += 1
    return content > 0 and titled == content


def is_unspoken_heading(text: str) -> bool:
    """Return True when *text* is a whole-paragraph chapter/section heading.

    Applied on the acoustic side only, like speaker/stage cues. A heading that
    shares a paragraph with prose is left alone.
    """
    ...
    if len(words) > HEADING_MAX_WORDS:
        return False
    core = _unwrap_heading_marks(raw)
    if not core:
        return False
    if HEADING_NUMBERED_ALLCAPS.fullmatch(core):
        return True
    quoted = raw[:1] in {'"', '“', "'", "‘"}
    if (
        not quoted
        and len(words) >= 3
        and HEADING_PROSE_END.search(core) is None
        and HEADING_ALLCAPS_LINE.fullmatch(core)
        and any(ch.isalpha() for ch in core)
    ):
        letters = re.sub(r"[^A-Za-z]", "", core)
        if letters and letters.isupper():
            return True
    if HEADING_SECTION_LABEL.match(core) and HEADING_PROSE_END.search(core) is None:
        return True
    if HEADING_LETTERED.match(raw) or HEADING_LETTERED.match(core) or HEADING_ROMAN.match(core):
        rest = re.sub(r"^_?(?:[A-Z]|[IVXLCM]+)\.\s+", "", raw)
        rest = _unwrap_heading_marks(rest)
        if HEADING_PROSE_END.search(rest) is not None:
            return False
        return bool(rest) and _looks_like_title(rest)
    if raw.startswith("_") and raw.endswith("_") and HEADING_PROSE_END.search(core) is None:
        return _looks_like_title(core)
    return False
...
def clean_text(text: str, *, strip_headings: bool = True) -> str:
    """Acoustic edition text: Kokoro-style cleanup plus cue/heading stripping.

    Heading detection is a whole-paragraph property. ``restore_source_tokens``
    incrementally re-cleans source prefixes; those prefixes must not re-enter
    the heading short-circuit (``strip_headings=False``), or a numbered
    first-person sentence such as ``45. I shall add…`` collapses at ``I``.
    """
    if strip_headings and is_unspoken_heading(text):
        return ""
    superscripts = "⁰¹²³⁴⁵⁶⁷⁸⁹"
    text = re.sub(f"[{re.escape(superscripts)}]+", "", text)
    text = WEB_EDITORIAL_BRACKET.sub(r"\1", text)
    text = STAGE_WRAP.sub(r"\1", text)
    ...
    # Keep ALL-CAPS names on Enter/Exeunt lines so expected_comparison_keys
    # can mark them unspoken. Title-casing here would hide the cue and also
    # break restore_source_tokens prefix mapping.
    if STAGE_DIRECTION_START.search(text.strip()) is None:
...
def normalize_token(token: str) -> str:
    # Underscore is a Python word character but never speech; drop it so
    # ``Hortensio._`` (what ASR often emits from ``[_…_]``) matches ``Hortensio``.
...
# ... ``]`` (``[28]``, ``[Greek: eunouchos]``, ``[Footnote: ...]``).  Revision 5
# also unwraps WEB editorial ``[of]`` / ``[and]`` and ``[_…_]`` stage wrappers;
# ALL-CAPS speaker labels, Enter/Exeunt names and whole-paragraph headings
# are dropped from the acoustic denominator separately (see ``clean_text`` /
# ``_mark_structural_cues``).
FOOTNOTE_MARKER = re.compile(r"\[(?:\d+|[A-Za-z]+:[^\[\]]*)\]")
FOOTNOTE_MARKER_OPEN = re.compile(r"\[(?:\d+|[A-Za-z]+:)[^\[\]]*$")
STAGE_WRAP_OPEN = re.compile(r"^\[_+")
STAGE_WRAP_CLOSE = re.compile(r"_+\]$")
WEB_EDITORIAL_TOKEN = re.compile(r"^\[([a-z][a-z ]*)\]$")
...
CONTRACTIONS = {
    ...
    "tis": "itis", "twas": "itwas", "twere": "itwere", "twould": "itwould",
    "thart": "thouart", "hearst": "hearest", "oer": "over",
}


def _is_allcaps_name(token: str) -> bool:
    core = re.sub(r"[^A-Za-z]", "", token)
    return bool(core) and core.isupper() and len(core) >= 2 and core not in STAGE_KEEP


def _looks_like_stage_direction(tokens: Sequence[str]) -> bool:
    joined = " ".join(tokens)
    return STAGE_DIRECTION_START.search(joined.lstrip()) is not None or joined.startswith("[_")


def strip_expected_markup(token: str) -> Tuple[str, bool]:
    """Return ``(stripped, changed)`` for one expected token.

    Removes underscore emphasis, complete footnote markers, WEB editorial
    brackets, ``[_…_]`` wrappers, and archaic ``'d`` / ``'st`` elisions used
    only for comparison.  Multi-token markers are handled by
    ``expected_comparison_keys``.
    """
    stripped = token
    editorial = WEB_EDITORIAL_TOKEN.fullmatch(stripped)
    if editorial:
        stripped = editorial.group(1)
    stripped = STAGE_WRAP_OPEN.sub("", stripped)
    stripped = STAGE_WRAP_CLOSE.sub("", stripped)
    stripped = WEB_EDITORIAL_BRACKET.sub(r"\1", stripped)
    stripped = stripped.replace("[_", "").replace("_]", "")
    stripped = ARCHAIC_SECOND.sub("est", stripped)
    stripped = ARCHAIC_PAST.sub("ed", stripped)
    stripped = stripped.replace("_", "")
    stripped = FOOTNOTE_MARKER.sub("", stripped)
    return stripped, stripped != token


def _mark_structural_cues(expected_tokens: Sequence[str], unspoken: List[bool]) -> None:
    """Drop printed cues the recogniser does not score as spoken words."""
    if not expected_tokens:
        return
    if is_unspoken_heading(" ".join(expected_tokens)):
        for index in range(len(unspoken)):
            unspoken[index] = True
        return
    if SUPERSCRIPT_TOKEN.fullmatch(expected_tokens[0] or ""):
        unspoken[0] = True
    if _is_allcaps_name(expected_tokens[0]):
        run: List[int] = []
        for index, token in enumerate(expected_tokens):
            if not _is_allcaps_name(token):
                break
            run.append(index)
            if token.endswith("."):
                for marked in run:
                    unspoken[marked] = True
                break
    if _looks_like_stage_direction(expected_tokens):
        for index, token in enumerate(expected_tokens):
            if _is_allcaps_name(token):
                unspoken[index] = True
...
def expected_comparison_keys(expected_tokens: Sequence[str]) -> List[Optional[str]]:
    ...
    # ordinary bracketed prose, list markers remain audible.
    if (
        ...
        unspoken[0] = True
        unspoken[1] = True
    _mark_structural_cues(expected_tokens, unspoken)
