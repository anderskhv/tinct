...
while time.monotonic()-start<45*60:
 r=subprocess.run([sys.executable,"tools/audio-highlight/runpod_guard.py","enforce","--owner-prefix",prefix,"--budget","15","--spent","2.60","--max-rate","1","--max-minutes","35","--json-out",str(root/"guard.json"),"--apply"],capture_output=True,text=True)
