...
for p in candidate["paragraphs"]:
 missing=[i for i,w in enumerate(p["words"]) if w["start"] is None or w["end"] is None]
 ...
 assert detail.stats.match_ratio>=.85
 for i in missing:
  assert i in detail.observed,(p["paragraph"],i,"missing span not observed")
  p["words"][i]=detail.words[i]
  repairs.append(dict(paragraph=p["paragraph"],word=i,span=detail.words[i],observedIndex=detail.observed[i]))
 p["nativeMissing"]=[]
(root/"native-gap-repairs.json").write_text(json.dumps(repairs,indent=1))
