...
4. **Deploy-after-verify is the default** until Tinct has more than 10 users (AGENTS.md Hard Rules, 2026-08-20). After `npm run build` and `npm run verify-bundle` pass, deploy with `npm run deploy` from `app/` using the Node 24 nvm path. Do not ask first unless Anders said local-only. Never run raw `wrangler deploy` or raw `vite build`. Never deploy from a dirty or unreconciled local checkout. Never deploy secrets. Never skip verify-bundle. When deploying a phase, still run the Pre-Deploy Checklist (`npm run dev` manual verify → `npm run build` → stash-build check → `npm run deploy` → `./scripts/smoke-test.sh` → exercise the change on https://tinct.app).
...
**Status 2026-06-15:** CODE COMPLETE; deployed before this documentation/test cleanup. Audit confirmed: 5.1 og:image exists; 5.2 is implemented with `/read/{bookId}` 404 noindex for unknown books, generic unknown-path noindex SPA fallback, and bare `/{bookId}` canonical metadata pointing to `/read/{bookId}`; 5.3 SEO surface is committed; 5.4 sitemap lastmod and `/read/{bookId}` Book JSON-LD are implemented; 5.5 `/read` serves the static crawlable hub and SEO clean URLs rewrite to static pages; 5.7 allows reputable AI crawlers while keeping junk scrapers blocked, with `robots.txt`, `llms.txt`, and `llms-full.txt` present. Regression coverage lives in `app/src/worker.seo.test.ts`.

Remaining operational SEO work:
- 5.6 is Anders-in-browser: Bing crawl-quota boost, IndexNow submit over the full sitemap, and record GSC/Bing indexed-vs-discovered health numbers in `SEO-STRATEGY.md`.
- Validate three live representative pages in Google's Rich Results test after the next deploy window.
