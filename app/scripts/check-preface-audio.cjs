...
(async()=>{const b=await chromium.launch();try{const p=await b.newPage({viewport:{width:1440,height:950}});
 ...
 const get=()=>p.evaluate(()=>({chapter:document.querySelector('.lab').dataset.chapter,place:document.querySelector('.lab').dataset.place,audio:[...document.querySelectorAll('audio')].filter(e=>e.currentSrc).map(e=>({src:e.currentSrc,time:e.currentTime}))}));
 const before=await get();await p.waitForTimeout(1500);assert.deepEqual(await get(),before);
