...
const output = 'artifacts/held-release-language-diagnostic'
...
async function pageState(page) {
  return page.getByTestId('lab-root').evaluate(root => {
    ...
    return {
      readerReady: root.dataset.readerReady,
      excerpt: visible.slice(0, 12).map(node => node.textContent).join(' '),
...
async function languageAcceptance(name, viewport) {
  ...
  const page = await context.newPage()
  const network = []
  page.on('response', response => { const url = new URL(response.url()); if (/great-expectations|\/assets\/(en-us|da)-/.test(url.pathname)) network.push({at:Date.now(),path:url.pathname,status:response.status()}) })
  let releaseEnglish
  ...
  const danishAt = Date.now()
  const danishKeys = danish.keys
  englishReleased = true
  releaseEnglish()
  const afterStaleEnglish = await stablePageState(page)
  await fs.writeFile(`${output}/${name}-diagnostic.json`, JSON.stringify({before,danish,danishAt,afterStaleEnglish,afterAt:Date.now(),network},null,2)+'\n')
...
try {
  for (const [name, viewport] of [
    ['desktop-a', { width: 1440, height: 900 }],
    ['desktop-b', { width: 1440, height: 900 }],
