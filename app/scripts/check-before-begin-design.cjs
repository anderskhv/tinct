...
assert.equal(await p.getByLabel('Secondary edition',{exact:true}).inputValue(),'original-en');
await p.getByRole('button',{name:'Select your editions',exact:true}).click();
const para=p.locator('.lab-preface-full p').first();const text=await para.innerText();const box=await p.locator('.lab-preface-full').boundingBox();const line=await para.evaluate(e=>parseFloat(getComputedStyle(e).lineHeight));if(name==='phone') assert(Math.abs(box.height-line*3)<3); else { assert(Math.abs(box.height-line*3)<3); const paragraphGap=await para.evaluate(e=>parseFloat(getComputedStyle(e).marginBottom)); assert.equal(await p.locator('.lab-preface-full p').count(),1); const cast=await p.locator('.lab-preface-cast').boundingBox(); assert(cast.x>box.x+box.width); }
...
results.push({name,prefaceLines:3,noRepeatedOpening:true,characters:true,questionDraftOnly:true,generationRequests:requests});await b.close();}fs.writeFileSync(out+'/checks.json',JSON.stringify(results,null,2));console.log(results)})().catch(err=>{console.error(err);process.exit(1)});
