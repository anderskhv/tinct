...
assert.equal(await p.getByLabel('Audiobook',{exact:true}).inputValue(),'');
await p.getByLabel('Audiobook',{exact:true}).selectOption('original-en');
assert.equal(await p.getByLabel('Primary edition',{exact:true}).inputValue(),'modern-en');
assert.equal(await p.getByLabel('Audiobook',{exact:true}).inputValue(),'original-en');
await p.screenshot({path:out+'/'+name+'-editions.png'});
await p.getByLabel('Audiobook',{exact:true}).selectOption('');
...
assert.equal(await p.getByLabel('Secondary edition',{exact:true}).inputValue(),'original-en');
await p.getByRole('button',{name:'Select your editions',exact:true}).click();
const para=p.locator('.lab-preface-full p').first();const text=await para.innerText();const box=await p.locator('.lab-preface-full').boundingBox();const line=await para.evaluate(e=>parseFloat(getComputedStyle(e).lineHeight));if(name==='phone') assert(Math.abs(box.height-line*3)<3); else { assert(box.height>line*3); const paragraphGap=await para.evaluate(e=>parseFloat(getComputedStyle(e).marginBottom)); assert.equal(await p.locator('.lab-preface-full p').count(),5); const cast=await p.locator('.lab-preface-cast').boundingBox(); assert(cast.x>box.x+box.width); }
if(name==='phone') await p.getByRole('button',{name:'Preface',exact:true}).click();assert.equal(await p.getByText(text,{exact:true}).count(),1);await p.screenshot({path:out+'/'+name+'-full-preface.png'});if(name==='phone') await p.getByRole('button',{name:'Preface',exact:true}).click();
if(name==='phone') await p.getByRole('button',{name:'Characters',exact:true}).click();
await p.getByRole('button',{name:/^Croesus/}).waitFor();
await p.getByRole('button',{name:/^Croesus/}).click();
await p.locator('.lab-preface-cast article').filter({has:p.getByRole('button',{name:/^Croesus/})}).getByText(/King of Lydia/).waitFor();
await p.screenshot({path:out+'/'+name+'-characters.png'});
...
results.push({name,prefaceLines:name==='phone'?3:'full',noRepeatedOpening:true,characters:true,questionDraftOnly:true,generationRequests:requests});await b.close();}fs.writeFileSync(out+'/checks.json',JSON.stringify(results,null,2));console.log(results)})().catch(err=>{console.error(err);process.exit(1)});
