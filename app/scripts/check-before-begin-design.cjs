...
await p.getByRole('button',{name:'Croesus',exact:true}).waitFor();
await p.getByRole('button',{name:'Croesus',exact:true}).click();
await p.locator('.lab-preface-cast article').filter({has:p.getByRole('button',{name:'Croesus',exact:true})}).getByText(/King of Lydia/).waitFor();
