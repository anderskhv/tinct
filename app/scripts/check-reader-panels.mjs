...
async function contentsAndSameEdition(engine,name,phone){
  ...
  try{
    ...
    if(phone){
      ...
      assert(Math.abs(bounds.x)<1&&Math.abs(bounds.y)<1&&Math.abs(bounds.width-viewport.width)<1&&Math.abs(bounds.height-viewport.height)<1,'phone contents owns the viewport: '+JSON.stringify({bounds,viewport}))
    }else assert(bounds.x>=0&&bounds.y>=0&&bounds.x+bounds.width<=1440&&bounds.height<520)
