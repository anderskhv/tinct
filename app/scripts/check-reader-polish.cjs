...
(async()=>{const b=await({chromium,webkit}[engine]).launch(),results=[];try{
 ...
 const audio=await setup(b,false,'modern-en');await audio.getByTestId('lab-v2-play').click();await audio.waitForFunction(()=>document.querySelector('.lab')?.dataset.playing==='true');await audio.waitForFunction(()=>document.querySelector('.lab-page-wrap > .lab-passage .is-current'));
