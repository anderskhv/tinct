...
const reviewedCases = {
  odyssey: [['odysseus',1,'odysseus',1],['nestor',3,'nestor',30],['eumaeus',14,'eumaeus',3]],
...
await p.waitForTimeout(350);await p.screenshot({path:`${dir}/${device}-${edition}-${label}.png`});await Promise.all(responseChecks);assert.ok(verifiedResponses.some(x=>x.startsWith(`/data/editions/${book}-${edition}.json?`)),'Reader must load versioned accepted text');assert.ok(verifiedResponses.some(x=>x.startsWith(`/data/characters/${book}.v1.json?`)),'Reader must load versioned character package');const shardManifest=`public/data/editions-chapters/${book}-${edition}/manifest.json`;if(fs.existsSync(shardManifest)){const entry=JSON.parse(fs.readFileSync(shardManifest,'utf8')).chapters.find(c=>c.number===ch);assert.ok(verifiedResponses.some(x=>x.startsWith(`/data/editions-chapters/${book}-${edition}/${entry.path}?`)),'Reader must load the matching accepted chapter shard')};await p.context().tracing.stop();results.push({device,edition,label,chapter:ch,paragraph:mention.paragraphIndex,id,place:before,verifiedResponses});await p.close()
