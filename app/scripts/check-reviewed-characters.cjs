...
if(device==='desktop')await word.click();else{const r=await word.boundingBox();await word.dispatchEvent('pointerdown',{pointerType:'touch',pointerId:1,button:0,buttons:1,isPrimary:true,bubbles:true,clientX:r.x+r.width/2,clientY:r.y+r.height/2});await p.waitForTimeout(400);await word.dispatchEvent('pointerup',{pointerType:'touch',pointerId:1,clientX:r.x+3,clientY:r.y+3})}
