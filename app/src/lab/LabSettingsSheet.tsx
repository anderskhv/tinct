...
import { LAB_FONT_FAMILIES, LAB_LIBRARY_URL, LAB_MAX_FONT_SIZE, LAB_MIN_FONT_SIZE, labAccountUrl, labReadingFont, labSignInUrl, type LabLineSpacing, type LabMargins, type LabFontFamily, type LabParagraphSpacing, type LabPrefs, type LabTextAlignment, type LabTheme } from './labPrefs'
...
/**
 * The face this sheet shows. It is the V1 sheet, so an unchosen face reads as
 * the face V1 has always set — the new default belongs to the new reader.
 */
function labSettingsFont(prefs: LabPrefs): LabFontFamily {
  return labReadingFont(prefs.fontFamily, false)
}
...
function HubRow({ icon, title, subtitle, onClick, href, testId }: { icon: string; title: string; subtitle: string; onClick?: () => void; href?: string; testId?: string }) {
  ...
  return href ? <a className="lab-ss-hub-row" href={href} onClick={onClick} data-testid={testId}>{content}</a> : <button type="button" className="lab-ss-hub-row" onClick={onClick} data-testid={testId}>{content}</button>
}
...
export interface LabSettingsSheetProps { open: boolean; section: SheetSection; onSection: (section: SheetSection) => void; onClose: () => void; prefs: LabPrefs; onPrefs: (prefs: LabPrefs) => void; editions: Edition[]; audioEditions: Edition[]; onOpenThisBook?: () => void; desktop?: boolean; /** Current reader path; the sign-in page returns here. */ returnTo?: string; /** Runs as the Library link is followed: the reader hands the library what it already knows. */ onLeaveToLibrary?: () => void }

export function LabSettingsSheet({ open, section, onClose, prefs, onPrefs, editions, audioEditions, onOpenThisBook, desktop = false, returnTo, onLeaveToLibrary }: LabSettingsSheetProps) {
  ...
  // Esc closes the sheet from anywhere in it, like the classic SettingsSheet.
  useEffect(() => {
    if (!open) return
  ...
  if (!open) return null
  const appearanceSummary = `${prefs.theme === 'system' ? 'Match system' : prefs.theme[0].toUpperCase() + prefs.theme.slice(1)} · ${labSettingsFont(prefs) === 'sourceserif' ? 'Source Serif' : labSettingsFont(prefs)[0].toUpperCase() + labSettingsFont(prefs).slice(1)} · ${Math.round((prefs.fontSize / 1.3) * 100)}%`
  ...
  if (view === 'hub') return <div className={`lab-ss-overlay is-hub${desktopClass}`} data-testid="lab-settings-sheet" onClick={onClose}><div className="lab-ss-hub" onClick={event => event.stopPropagation()}><HubRow icon="⌂" title="Library" subtitle="Return to your books" href={LAB_LIBRARY_URL} onClick={onLeaveToLibrary} testId="lab-settings-library" /><HubRow icon="Aa" title="Appearance & Text" subtitle={appearanceSummary} onClick={() => setView('appearance')} testId="lab-settings-layout" /><HubRow icon="◉" title="Audio & Editions" subtitle={primaryLabel} onClick={() => setView('audio')} testId="lab-settings-reading" /><HubRow icon="○" title="Account" subtitle="Profile, AI credits, and reading history" onClick={() => setView('account')} testId="lab-settings-account" />{onOpenThisBook && <button className="lab-ss-legacy-action" data-testid="lab-in-the-book" onClick={onOpenThisBook}>This book</button>}<button className="lab-ss-legacy-action" data-testid="lab-compare" onClick={() => onPrefs({ ...prefs, compareOpen: !prefs.compareOpen })}>Compare</button></div></div>
  ...
  return <div className={`lab-ss-overlay${desktopClass}`} data-testid="lab-settings-sheet" onClick={onClose}><section className="lab-ss-sheet" onClick={event => event.stopPropagation()} aria-label={title}><span className="lab-ss-grabber" /><div className="lab-ss-head"><button type="button" className="lab-ss-back" onClick={() => setView(view === 'all' ? 'appearance' : 'hub')} aria-label="Back">←</button><h2 className="lab-ss-title">{title}</h2><button type="button" className="lab-ss-close" onClick={onClose} aria-label="Close settings">×</button></div><div className="lab-ss-body">
    ...
    {view === 'account' && <><div className="lab-ss-account-card"><strong>{auth.user?.email || 'Reader account'}</strong><span>{auth.user ? (balance.isSubscribed ? 'Premium account' : 'Free account') : 'Reading locally'}</span></div><Row label="AI credits remaining" control={<strong>{auth.user ? balance.messagesRemaining.toLocaleString('en-US') : '—'}</strong>} /><div className="lab-ss-year"><small>This year</small><div><span><strong>{Math.max(0, Number(localStorage.getItem('tinct-lab-reading-seconds') || 0)) < 3600 ? '<1' : Math.round(Number(localStorage.getItem('tinct-lab-reading-seconds') || 0) / 3600)}</strong> hours</span><span><strong>{Math.max(0, Number(localStorage.getItem('tinct-lab-page-turns') || 0)).toLocaleString('en-US')}</strong> pages</span><span><strong>{auth.user ? '1' : '—'}</strong> books</span></div></div>{auth.user
      ? <a className="lab-ss-deep-link" href={labAccountUrl(returnTo)} data-testid="lab-account-manage">Manage account <Arrow /></a>
      : <a className="lab-ss-deep-link" href={labSignInUrl(returnTo)} data-testid="lab-account-sign-in">Sign in or create a free account <Arrow /></a>}{auth.user?.email && <button type="button" className="lab-ss-reset" onClick={async () => { const result = await auth.resetPassword(auth.user!.email!); setResetStatus(result.error || 'Password reset email sent.') }}>Reset password</button>}{resetStatus && <p className="lab-ss-reset-status" role="status">{resetStatus}</p>}</>}
  </div></section></div>
}
