...
export function VoiceLab() {
  ...
  const exportTest = () => {
    const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), voiceModel: 'gpt-live-1', timing: 'Client event receipt times; useful-answer marks are manual and include observer delay.', events }, null, 2)], { type: 'application/json' })
    ...
    {open && <aside className="voice-lab-panel" aria-label="Voice test controls">
      ...
      {tab === 'settings' ? <>
        <fieldset disabled={active}>
          ...
          <label>Speaking prompt<textarea rows={9} maxLength={16000} value={draft.frontend} onChange={e => setDraft({ ...draft, frontend: e.target.value })} /></label>
