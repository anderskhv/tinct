    ...
    {open && <aside className="voice-lab-panel" aria-label="Voice test controls">
      ...
      {tab === 'settings' ? <>
        <fieldset disabled={active}>
          ...
          <label>Preset<select aria-label="Preset" value="" onChange={e => { const presets_ = [STREAMED_VOICE_EXPERIMENT, CURRENT_VOICE_EXPERIMENT, LIGHT_VOICE_EXPERIMENT, INSIGHT_VOICE_EXPERIMENT, INSIGHT_REASONED_EXPERIMENT, ...presets]; const selected = presets_[Number(e.target.value)]; if (selected) setDraft({ ...selected }) }}><option value="" disabled>Choose a preset…</option>{[STREAMED_VOICE_EXPERIMENT, CURRENT_VOICE_EXPERIMENT, LIGHT_VOICE_EXPERIMENT, INSIGHT_VOICE_EXPERIMENT, INSIGHT_REASONED_EXPERIMENT, ...presets].map((p, i) => <option key={i} value={i}>{p.label}</option>)}</select></label>
