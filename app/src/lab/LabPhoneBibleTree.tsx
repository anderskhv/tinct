...
export function LabPhoneBibleTree({ title, chapters, currentChapter, sections, finishedChapters, statuses, highlights = [], conversations = [], onSelectChapter, onSelectHighlight, onOpenConversation, onNewConversation, onWarmChapter, onClose }: Props) {
  ...
  // Highlights and conversations sit under their chapter row — in a Bible
  // book's chapter list and in a flat contents list alike.
  const annotationsFor = (chapterNumber: number) => {
    const hs = highlights.filter(item => item.chapterNumber === chapterNumber)
    const cs = conversations.filter(item => item.chapterNumber === chapterNumber)
    return <>{hs.map(item => <button className="lab-map-annotation" key={item.id} onClick={() => onSelectHighlight?.(item)}>▰ <span>{item.note || 'Highlighted passage'}</span></button>)}{cs.length > 0 && <button className="lab-map-annotation" onClick={() => setConversationChapter(chapterNumber)}>◯ <span>{cs.length} conversation{cs.length === 1 ? '' : 's'}</span></button>}</>
  }
  ...
  return <div className="toc-overlay lab-tree" data-testid="lab-bible-tree"><div className="toc-panel lab-tree-panel">
    ...
    <main className="lab-map-body" ref={bodyRef}>
      ...
      : <section>{path.length > 0 && <button className="lab-map-crumb" onClick={() => setPath(path.slice(0, -1))}>← {path.length > 1 ? path[path.length - 2].title : 'Contents'}</button>}<div className="lab-map-section-title"><h3>{active?.title || 'Contents'}</h3>{active && <small>{labTreeProgressLabel(active, finishedChapters) || `${active.chapterNumbers.length} chapters`}</small>}</div>
        {visibleNodes.filter(node => passes(node.chapterNumbers)).map(node => node.kind === 'chapter' ? <div className={`lab-map-chapter${node.chapterNumber === currentChapter ? ' is-current' : ''}`} key={node.key} data-current={node.chapterNumber === currentChapter ? 'true' : undefined}><button className="lab-map-chapter-main lab-tree-chapter toc-item" data-testid={`lab-tree-chapter-${node.chapterNumber}`} aria-current={node.chapterNumber === currentChapter ? 'true' : undefined} onPointerEnter={() => onWarmChapter?.(node.chapterNumber!)} onClick={() => onSelectChapter(node.chapterNumber!)}><span><strong>{node.title}</strong><small>{statusLine(node.chapterNumber!)}</small></span><b>›</b></button>{annotationsFor(node.chapterNumber!)}</div> : <button className="lab-map-row lab-tree-row toc-section-header" key={node.key} data-kind={node.kind} onClick={() => setPath([...path, node])}><span><strong>{node.title}</strong><small>{labTreeProgressLabel(node, finishedChapters) || (node.kind === 'book' ? `${node.chapterNumbers.length} chapters` : `${node.children?.length || 0} books`)}</small></span><span className="lab-map-counts">{countFor(node, 'highlights') > 0 && `▰ ${countFor(node, 'highlights')}`}{countFor(node, 'chats') > 0 && ` ◯ ${countFor(node, 'chats')}`} <b>›</b></span></button>)}
        {bookChapters.filter(chapter => passes([chapter.number])).map(chapter => { return <div className={`lab-map-chapter${chapter.number === currentChapter ? ' is-current' : ''}`} key={chapter.number} data-current={chapter.number === currentChapter ? 'true' : undefined}><button className="lab-map-chapter-main lab-tree-chapter toc-item" data-testid={`lab-tree-chapter-${chapter.number}`} aria-current={chapter.number === currentChapter ? 'true' : undefined} onPointerEnter={() => onWarmChapter?.(chapter.number)} onClick={() => onSelectChapter(chapter.number)}><span><strong>{chapter.title}</strong><small>{statusLine(chapter.number)}</small></span><b>›</b></button>{annotationsFor(chapter.number)}</div> })}
      </section>}
