...
export function LabChapterHeading({ title, preview = false, busy = false, onPreview }: {
  title: string; preview?: boolean; busy?: boolean; onPreview?: () => void
}) {
  return <header className={`lab-passage-header${preview ? ' has-preview' : ''}`}>
    <h1 className="lab-passage-headline" data-testid="lab-passage-headline">{title}</h1>
