...
describe('lab/index.html boot script', () => {
  ...
  it('paints the account and the cached recap for the same account, in the confirmed render\'s markup', () => {
    ...
    expect(recap.querySelector('.lib-recap-hero')?.getAttribute('data-recap-hero')).toBe('bible')
    expect(recap.querySelector('.lib-recap-head .lib-eyebrow')?.textContent).toBe('Last time you read · Proverbs 17')
    expect(recap.querySelector('.lib-recap-head .lib-h1')?.textContent).toBe('You stopped in Proverbs 17')
    expect(recap.querySelector('.lib-recap-meta .lib-lede')?.textContent).toBe('The Bible')
    expect(recap.querySelector('[data-recap-continue]')?.getAttribute('data-recap-continue')).toBe('bible')
    expect(recap.querySelector('.lib-recap-cover img')?.getAttribute('src')).toBe('/covers/bible.jpg')
