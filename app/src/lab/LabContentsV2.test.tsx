...
it('puts Cover and preface above the chapter tree without choosing a chapter', () => {
  const p=props(), open=vi.fn()
  render(<LabContentsV2 {...p} onOpenCover={open}/>)
  const button=screen.getByRole('button',{name:'Cover and preface'})
  const tree=screen.getByRole('navigation',{name:'Chapters'})
  expect(tree.firstElementChild?.contains(button)).toBe(true)
