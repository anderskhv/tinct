...
let mode: Mode = allowedModes.has(new URLSearchParams(location.search).get('mode') as Mode)
  ? new URLSearchParams(location.search).get('mode') as Mode
  : 'signin'
...
function setMode(next: Mode) {
  ...
  url.searchParams.set('returnTo', '/lab/library-2')
  ...
  try {
    ...
    if (mode === 'signin') {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...values,
        options: { emailRedirectTo: `${location.origin}/lab/sign-in?returnTo=%2Flab%2Flibrary-2` },
      })
      if (error) throw error
      if (data.session) {
        setSignedInCookie()
      ...
      const { error } = await supabase.auth.resetPasswordForEmail(values.email, {
        redirectTo: `${location.origin}/lab/sign-in?mode=reset&returnTo=%2Flab%2Flibrary-2`,
      })
      if (error) throw error
...
root?.addEventListener('click', async event => {
  ...
  if (target.hasAttribute('data-google') && supabase) {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${location.origin}/lab/sign-in?returnTo=%2Flab%2Flibrary-2` },
    })
    if (error) {
