import {authorPortrait,warmPortrait} from './authors.js?v=20260925j';
import {readingRoom,sceneAsset,tableCrop} from './reading-room.js?v=20260925j';
import {books} from './books.js?v=20260925j';
import {loadCatalogue,libraryBook,attachCatalogue,loadIntroduction,readerDestination,readingApi} from './catalogue.js?v=20260925j';
import {drawSceneLife,scenePainting} from './scene-life.js?v=20260925j';
import {categories,eras,metadata} from './taxonomy.js?v=20260925j';
import {clamp,ease,mix,destination,bookFrame,orbFrame,dockPosition,sceneCrop,panelBounds} from './motion.js?v=20260925j';
...
loadCatalogue().then(({books:entries,houses})=>{const byId=new Map(entries.map(e=>[e.id,e]));books.forEach(book=>{const entry=byId.get(book.id);if(entry)attachCatalogue(book,entry);});entries.forEach(entry=>{if(books.some(b=>b.id===entry.id))return;const book=libraryBook(entry);attachCatalogue(book,entry);books.push(book);});
 ...
 if(searchOpen)renderResults();if(collectionChoice!=='home')renderCollection();}).catch(()=>{});
...
$('book-backdrop').onclick=$('tour-back').onclick=closeBook;$('back-library').onclick=()=>{if(currentTab!=='preface'){currentTab='preface';renderIntro();}else if(bookDestination().tour){if(!tourAnimation)animateTour(0).then(()=>$('slip-next').focus({preventScroll:true}));}else{$('book-slip').scrollTop=0;$('book-slip').focus({preventScroll:true});}};
...
async function loadAssistant(){
 ...
 assistantLoading=import('/lab/library-2-assistant.js?v=20260925j').then(()=>window.__tinctLibraryTwoAssistant.mount($('librarian-live'),{
...
import('./reading-table.js?v=20260925j').then(m=>m.mountReadingTable({hero:$('hero'),shelves:$('shelves'),el})).catch(()=>{});
