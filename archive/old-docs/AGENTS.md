...
This project is part of Anders's portfolio, managed by `AGENTS.md` (Group CEO) at the Documents root. This AGENTS.md is the project CEO — it owns this project end-to-end.

**IMPORTANT: You are a project CEO, NOT the Group CEO.** Do NOT run the session protocol from the root AGENTS.md (no Garmin sync, no calendar check, no morning check-in, no time tracking). That is handled by the Group CEO in the root Documents folder. You focus only on this project's code and tasks.
...
All content generation — book editions, translations, chapter summaries, any text produced by Codex — must happen through the CLI conversation and be written to files. **Never** run scripts that call `api.anthropic.com` during development.
...
- **If Codex API fails**: Degrade gracefully — reading experience should work without AI features.
...
- Spending money (Codex API calls beyond dev/test, services)
...
**[Content Model 2026-03-16]**: Pre-compute all versions via Codex (modern EN, kids EN, modern DA, kids DA). Store as static data. ~$200-350 for 20 books. One-time cost, lowest per-user cost long-term.
...
**[Chat Markdown 2026-03-17]**: Full markdown rendering in chat and notes — headings (h1-h3), bold, italic, bullet lists, numbered lists. Codex responses look formatted, not raw.
