...
Status: the 23 September Grok implementation uses Ara/Helios defaults and optional Orion/Eve. Its preparation, acceptance and cutover receipts are tracked in [Grok release evidence](grok-narration-2026-09-23.md). The 21 September WaveNet/Bella sections below are historical evidence; physical-device lock-screen follow-up remains separate. `fish-audio-pilot-2026-09-18.md` remains historical evidence for the cache, lock and spend-control pilot that this design adapts.

## Grok rollout contract — 23 September 2026

Anders selected Grok **Ara** (`ara`, default female) and **Helios** (`helios`, default male), with **Orion** (`orion`, optional male) and **Eve** (`eve`, optional female). These are audiobook voices; do not change Talk/conversation voice selection. No additional voices are in scope. Preserve the current on-demand generation, shared content-addressed cache, bounded look-ahead, stop-on-pause scheduling and spending controls; do not pre-generate both voices across all books and editions.
...
The full source-dependent modernization assessments and cautions are maintained in [Strategy: featured ten-book selection](../STRATEGY.md#featured-ten-book-selection). This list replaces the earlier proposal to use the first ten of the existing popular shelf; the runtime shelf now follows this order.

**Final approved preparation budget:** approximately five minutes in each of the two default voices for each of the ten books above, initially only its actual default entry English edition: approximately 100 minutes total. This supersedes the earlier fifteen-minute proposal. Determine exact edition IDs and initial reader positions from the current app and record them before generation; preserve editorial holds and do not silently substitute editions. Opening generation is restricted to the bounded cloud release workflow.

Outside those prepared openings, synthesize only after explicit Play. Browsing, opening or silently reading a nonfeatured book must generate nothing. Orion and Eve are available on demand and may show loading, including on featured books; they are not pre-generated. Maintain a small rolling buffer only during playback and stop new scheduling on pause, leave, voice or edition change while retaining valid completed cache.

Reuse valid small-section recordings and timing data across readers and matching editions using exact text/provider/model/voice/settings identity, selective invalidation, concurrency deduplication, bounded retries, usage tracking and spending ceilings. Do not generate whole chapters on opening or synthesize skipped material when seeking.

Acceptance must measure real provider requests with timestamps: prepared default-voice starts for the ten exact editions, continuous playback past five minutes, cold first-audio latency (target at most five seconds, not a promise until measured), repeated/concurrent reuse, optional-voice cache separation, no synthesis on nonfeatured browse, highlighting, estimated whole-chapter timelines and uncached seeks, pause/leave and chapter/Media Session regressions. Arbitrary later-chapter seeks are not pre-recorded. Report physical-device limits candidly.

Retire production WaveNet and Kokoro/Bella selection/fallback paths at the accepted Grok cutover. Retain their assets briefly for rollback, then delete only inventoried obsolete recordings/timings/maps after migration acceptance and required preservation checks; never blanket-delete a bucket or unrelated languages.

### Text publication is a separate release

Anders explicitly authorized accepted edition replacements with required character-card compatibility, without waiting for legacy Kokoro regeneration. The Frankenstein/Jekyll text release includes no GPU/TTS generation, audio regeneration or voice changes. Record exact accepted/served text hashes, unchanged structure, card revisions and any conservative omitted aliases. Older audio-regeneration instructions in release packets or book guides are superseded for this release. This does not waive text/audio identity and synchronization requirements in the separately approved narration migration.

The sections below document the superseded implementation and historical acceptance evidence. Current voice mapping, atomic Durable Object leases/spend reservations, sparse seek maps, Play-only scheduling and failure behavior are described in the Grok release document.
