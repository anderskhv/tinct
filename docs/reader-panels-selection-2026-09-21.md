...
Candidate implementation keeps the full-height book fold inside the actual column gap. Saved-highlight words, spaces and hyphen fragments carry the same topmost record identity as their colour; clicks use that identity before dictionary lookup, including projected editions. Tiny fractional row cracks receive an underlay scoped to the shared horizontal part of adjacent highlighted rows; it neither changes typography nor bridges deliberate line/paragraph spacing. The switcher focus outline is inset, the phone menu follows the measured header with a 12px gap, and phone progress uses the same quiet opacity and ink as the title. Reading/navigation/storage schemas and source content are unchanged.

Acceptance adds reopened Gold records for Matthew 6 WEB and Notes from Underground, first/middle/last and “can” clicks, persisted deletion, overlap identity, Retina paint sampling, fold-versus-ink bounds, and phone menu/focus/quiet-colour checks in Chromium and WebKit. Candidate and production evidence will be recorded here after completion. Initial diagnostics exposed gaps in the test fixtures (walking beyond the text leaf and omitting a unit fixture’s page); those fixtures were corrected before acceptance. The reported user record itself is not available in the isolated browser, so these checks use representative persisted records and the supplied passages.
