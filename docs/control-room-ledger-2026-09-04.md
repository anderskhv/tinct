...
| 5 Integration + release | `codex/claude-convergence-integration` | deployed (releases 1, 1.1, 2) | Release 2 `ef6ba7ec` deployed 07:08 UTC: Worker `c3e3e696-6aaf-408f-9a25-c848a552dcd1`, bundle `index-DWzW5uk-.js`; control room re-verified bundle + library script from a second container; `codex/lab-convergence` fast-forwarded to `ef6ba7ec`. |
...
| 9 Whisper text biasing | `codex/claude-word-timing-production` @ `/home/user/tinct-wt/word-timing` | implementing | Owner "build 1". `initial_prompt`/`hotwords` from paragraph text in `words_sidecar_lib.py`, measured on CPU against Genesis 5/10 WEB and Genesis 10 Modern vs Genesis 2 control; additive provenance `alignment.bias`; backward-compatible CLI for the second pass. |
| 10 Reader sentence fallback | `codex/claude-lab-follow-fallback` | integrated (release 2.1 deploying) | `08f47288` on `ef6ba7ec`, 6 files under `app/src/lab/`. Paragraphs with sidecar `alignment.matchRatio` below the chapter threshold (default 0.85) follow at sentence granularity (`data-follow-granularity="sentence"`), boundaries at `. ! ? ; :` and verse markers; word-level unchanged elsewhere; missing alignment = word-level. Focused 193/193, full 987/987, build + verify-bundle pass, browser smoke pass on desktop + phone with an edited-ratio Genesis 1 fixture. Not verified: real audio, a genuine sub-0.85 chapter. Fast-forwarded into the integration branch; deploy session started. |
