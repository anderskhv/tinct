...
| 2 Word-timing canary | `codex/claude-word-timing-production` | **production verified** (canary) / blocked (GPU benchmark) | Rerun 3 with `CF_R2_TOKEN` (custom token: Account · Workers R2 Storage · Edit): bucket metadata 200, known-object read OK, both Genesis 1 pilots uploaded with no-overwrite check, GET/hash/schema/provenance verified, production serves `bible/web-en/ch1/words.json` (72,454 B) and `bible/modern-en/ch1/words.json` (72,226 B) at 200, headless highlighting check passes at 1× and 2× (word identity follows media time; not perceptual). Report `5462d952`. Learned: R2-page tokens fail wrangler's REST path with 10000; a Workers R2 Storage token works. Rerun 2 accidentally used the 05:31 R2-page token. Calibration measured (`b8d23149`): ≈20× real time on an RTX 4090, 18/18 sidecars valid, 3 genealogy chapters under the 85 % gate. Decisions 2026-09-05: no Danish; four Community pods (~$40); GrokBot runs Shards 1–4 with `--upload` via a RunPod Secret `CF_R2_TOKEN`. Open: Anders's go/hold on (1) Whisper initial-prompt biasing and (2) reader sentence-level fallback for paragraphs under 85 %. |
| 3 Reading memory + recap | `codex/claude-reading-memory-recap` | integrated (release 2 deploying) | Cherry-picked onto `923873db` as `2ec48955` + `ffea6452` with no conflicts; control room added `ef6ba7ec` (skip the pre-adoption `reading-memory` snapshot in the legacy sign-in migration; closes a race Lane 5 found where an `owner: null` snapshot could become the account's cloud row). Focused 318/318 + 207/207 after fix, full 975/976 (catalogue flake passes alone), build + verify-bundle (`index-PvZX36dd.js`), smoke 10/10. Deploy session started. |
...
| 5 Integration + release | `codex/claude-convergence-integration` | deployed (1.1) / deploying (release 2 at `ef6ba7ec`) | Release 1.1 live (`923873db`). Release 2 integrated `ffea6452` + fix `ef6ba7ec`; deploy session running via `CI=true npm run deploy`. |
