...
| 5 Integration + release | `codex/claude-convergence-integration` | deployed (releases 1, 1.1, 2, 2.1, 3) | Latest: release 3 (`a2a271aa`, locked library) live; convergence promoted. |
...
| 12 Library round 2 | `codex/claude-lab-library-round2` @ `/home/user/tinct-wt/library2` | implementing | Anders's phone feedback on release 3 (2026-09-05 evening): theme carry-over from landing, tighter mobile paddings, harmonised Popular label, no-jump shelf, titles under every cover, longer descriptions from registry, search→back state restore, house cover grids (2 cols mobile), account icon on mobile, Reading now + Finished sections for returning readers, and a bug where Continue opened Daniel 7 instead of the last read (memory session vs position store). Open decisions: cut the Popular label?; integrate all 100 covers now or after the 8 redos. |
