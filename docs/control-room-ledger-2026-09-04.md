...
| 10 Reader sentence fallback | `codex/claude-lab-follow-fallback` @ `/home/user/tinct-wt/follow-fallback` | implementing | Owner "build 2". Base `ef6ba7ec`. Paragraphs with sidecar `alignment.matchRatio` < 0.85 follow at sentence granularity in `labFollow.ts`; word-level unchanged elsewhere. |
