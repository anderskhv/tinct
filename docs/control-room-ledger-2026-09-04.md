...
| 11 Locked library design | `codex/claude-lab-library-locked` | integrated (release 3 deploying) | `ab4ee98a` (ten v2 cover WebPs, 1.3 MB total, sourced from cover branch `8ab9fc3c`) + `a2a271aa` (locked library: shelf with selection/reveal/keyboard, returning hero from reading memory, hairline search filtering the index, house index with counts, old library panels removed; e2e rewritten 47/47). Full 1004/1004, build + verify-bundle pass, screenshots match the four artboards; deviations documented (real counts 100, Bible replaces Meditations on the shelf, reveal ≈1.1 s). Control room reviewed the four screenshots. |
