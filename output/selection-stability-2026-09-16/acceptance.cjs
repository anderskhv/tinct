...
const loading=await p.locator('.selection-popup').boundingBox();await p.screenshot({path:out+'/'+name+'-loading.png'});release();await p.locator('.lab-contextual-explain strong em').waitFor();
...
await p.screenshot({path:out+'/'+name+'-ready.png'});assert.deepEqual(errors,[]);
results.push({name,bundle:await p.locator('script[src*="assets/index-"]').getAttribute('src'),nativePrevented,readerUnchanged:true,nativeSelectionEmpty:true,ranges,loading,ready,chat,talk,scroll,errors});
